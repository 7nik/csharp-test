...
namespace SimpleGameBookmaker
{
    ...
    class SimpleGameBookmaker
    {
		...
		private SimpleGameRecord[] records;

		public SimpleGameBookmaker(SimpleGameRecord[] records)
		{
			this.records = records;
		}

		// get array of all team names presented in the records
		public string[] GetTeamNames()
		{
			return records
				.Select(rec => rec.HomeTeamName)
				.Concat(records.Select(rec => rec.AwayTeamName))
				.Distinct()
				.OrderBy(name => name)
				.ToArray();
		}

		// get N last game records of a team
		public SimpleGameRecord[] GetLastGamesOfTeam(string teamName, int count)
		{
			return records
				...
				.OrderBy(rec => rec.GameDate)
				.Take(count)
				.ToArray();
		}

		// get count of won, drawn, and lost games for the N last games of a team
		public (int won, int drawn, int lost, int total) GetStatsOfTeam(string teamName, int gameCount)
		{
			return GetLastGamesOfTeam(teamName, gameCount).Aggregate((won: 0, drawn: 0, lost: 0, total: 0), (stats, rec) => {
		...
		// calculated odds of match a home team vs an away team
		public (double win, double draw, double lose) CalculatedOdds(string homeTeamName, string awayTeamName)
		{
			var homeTeamStats = GetStatsOfTeam(homeTeamName, 15);
			var awayTeamStats = GetStatsOfTeam(awayTeamName, 15);
        ...
        {
			SimpleGameBookmaker bookmaker = new SimpleGameBookmaker(SimpleGameBookmaker.sampleRecords);

			Console.WriteLine("List of available teams:");
			string[] teamNames = bookmaker.GetTeamNames();
			...
			var (win, draw, lose) = bookmaker.CalculatedOdds(teamNames[homeTeamNumber], teamNames[awayTeamNumber]);

			Console.WriteLine();
			//Console.WriteLine($"Match odds:\n{teamNames[homeTeamNumber]} at {win}\nDraw at {draw}\n{teamNames[awayTeamNumber]} at {lose}");
			Console.WriteLine("Match odds:\n{0} at {2:f2}\nDraw at {3:f2}\n{1} at {4:f2}", teamNames[homeTeamNumber], teamNames[awayTeamNumber], win, draw, lose);
			Console.ReadLine();
		}
    }
}
